...
export class BorrowedBooksService {
    ...
    async create(createBorrowedBookDto: CreateBorrowedBookDto) {
        ...
        if (!book) {
            throw new NotFoundException(`Book with ID ${createBorrowedBookDto.bookId} not found`);
        }

        if (!book.available) {
            throw new BadRequestException('This book is currently not available for borrowing');
        }
        ...
        if (!user) {
            throw new NotFoundException(`User with ID ${createBorrowedBookDto.userId} not found`);
        }
    ...
    async returnBook(id: string) {
        ...
        if (!borrowedBook) {
            throw new NotFoundException(`Borrowed book record with ID ${id} not found`);
        }
